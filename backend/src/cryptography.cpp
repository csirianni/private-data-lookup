...
std::string hashAndEncryptPassword(const std::string &password,
                                   unsigned char *b, size_t offset) {
...
std::string encryptPassword(const std::string &password, unsigned char *b,
    ...
    std::string raw_password =
        password.substr(offset, crypto_core_ristretto255_BYTES);
    ...
    return std::string(encrypted_password,
                       encrypted_password + crypto_core_ristretto255_BYTES);
}

std::vector<std::string> encrypt(
    const std::unordered_set<std::string> &passwords, unsigned char *b,
    size_t offset) {
