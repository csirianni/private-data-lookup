...
/**
 * @brief Generate a set of passwords where each password contains at least one
 * lowercase letter, at least one uppercase letter, at least one digit, and at
 ...
 */
std::unordered_set<std::string> generatePasswords(int num_passwords,
                                                  int max_chars);
}  // namespace password
#endif  // PASSWORDS_H
