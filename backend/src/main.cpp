...
int main(int argc, char *argv[]) {
    ...
    if (file_path.find(".db") != file_path.length() - 3) {
        throw std::invalid_argument(
            "File does not end in .db. Use the correct file extension to open "
            "the database.");
    }

    database::Database db = database::Database(argv[1], build);
    // TODO: store this data in server because same file with different offset
    ...
    if (build) {
        ...
        for (const auto &password : encrypted_passwords) {
            ...
            // unsigned char cast is required to prevent unintentional sign
            // extension
            // TODO: support more than one byte using helper function
            unsigned int table_num = static_cast<unsigned int>(
                static_cast<unsigned char>(password.substr(0, offset)[0]));
            std::string raw_password =
                password.substr(offset, password.size() - offset);
            // encode password before inserting into database
            db.execute(
                "INSERT INTO `" + std::to_string(table_num) +
                "` (password) VALUES ('" +
        ...
        db.execute("INSERT INTO offset (key) VALUES (" +
                   std::to_string(offset) + ");");
    } else {
        // error check if !build but secret table does not exist in the file
        // passed in
        ...
        std::vector<bool> secret_key = db.execute(
            "SELECT COUNT(*) FROM sqlite_schema WHERE name = 'secret';",
            callback);
        std::vector<bool> offset_key = db.execute(
            "SELECT COUNT(*) FROM sqlite_schema WHERE name = 'offset';",
            callback);
        if (secret_key.front() == 0 ||
            offset_key.front() ==
                0)  // no secret key or no offset key table exists
        {
            throw std::invalid_argument(
                "Secret key table or offset key table does not exist. Use "
                "--build to create a new database");
        }
    }
    // Enable CORS
