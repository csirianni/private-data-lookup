...
class Database {
   public:
    ...
    /**
     * @brief Execute the provided SQL command and load each row into a vector
     * using the callback function.
     *
     * @tparam T The type of the vector to be returned.
     * @param command The command to be executed.
     * @param callback The callback function to be used to load each row into
     * the vector.
     * @return std::vector<T> The vector containing the rows of the table.
     */
    template <typename T>
    std::vector<T> execute(const std::string &command,
                           std::function<T(sqlite3_stmt *)> callback) {
        sqlite3_stmt *stmt;
        if (sqlite3_prepare_v2(db_, command.c_str(), -1, &stmt, NULL) !=
            SQLITE_OK) {
