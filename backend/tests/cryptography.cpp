...
TEST_CASE("Test hashAndEncryptPassword") {
    ...
    crypto_generichash(expected_hash, sizeof expected_hash,
    ...
    SECTION("without leaked byte") {
        for (int i = 0; i < 30; ++i) {
            // encrypt password
            std::string expected_password_str =
                cryptography::hashAndEncryptPassword(password, b);
            unsigned char encrypted_password[crypto_core_ristretto255_BYTES];
            memcpy(encrypted_password, expected_password_str.data(),
                   crypto_core_ristretto255_BYTES);

            // unencrypt the password with the inverse of b
            unsigned char decrypted_password[crypto_core_ristretto255_BYTES];
            int result = crypto_scalarmult_ristretto255(
                decrypted_password, inverse, encrypted_password);
            REQUIRE(result == 0);

            CHECK(std::memcmp(expected_point, decrypted_password,
    ...
    SECTION("with 1 leaked byte") {
        for (int i = 0; i < 30; ++i) {
            ...
            // encrypt password
            std::string encrypted_password_str =
                cryptography::hashAndEncryptPassword(password, b, offset);
            unsigned char
                encrypted_password[crypto_core_ristretto255_BYTES + offset];
            memcpy(encrypted_password, encrypted_password_str.data(),
                   crypto_core_ristretto255_BYTES + offset);

            // decrypt password
            unsigned char
                decrypted_password[crypto_core_ristretto255_BYTES + offset];
            int result = crypto_scalarmult_ristretto255(
                decrypted_password + offset, inverse,
                encrypted_password + offset);
            REQUIRE(result == 0);
            memcpy(decrypted_password, encrypted_password, offset);

            CHECK(std::memcmp(expected_point, decrypted_password + offset,
                              crypto_core_ristretto255_BYTES) == 0);
