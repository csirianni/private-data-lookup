...
TEST_CASE("Test Database class") {
    ...
    SECTION("Create, insert, and select queries") {
        ...
        for (const auto &name : names) {
            REQUIRE_NOTHROW(db.execute("INSERT INTO names (name) VALUES ('" +
                                       name + "');"));
        }

        // check that the names are in the database
        std::function<std::string(sqlite3_stmt *)> callback =
            [](sqlite3_stmt *stmt) {
                return std::string(reinterpret_cast<const char *>(
                    sqlite3_column_text(stmt, 0)));
            };
        std::vector<std::string> result =
            db.execute("SELECT * FROM names;", callback);
        CHECK(result == names);
        ...
        SECTION("Access persistent storage in database") {
            ...
            std::function<int(sqlite3_stmt *)> callback_names_table =
                [](sqlite3_stmt *stmt) {
                    int count = atoi(reinterpret_cast<const char *>(
                        sqlite3_column_text(stmt, 0)));
                    return count;
                };
            std::vector<int> result_names_table = reopen_db.execute(
                "SELECT COUNT(*) FROM sqlite_schema WHERE name = 'names';",
                callback_names_table);
            CHECK(result_names_table.front() == 1);

            // check if the previously inserted names are still there
            std::function<std::string(sqlite3_stmt *)> callback_each_name =
                [](sqlite3_stmt *stmt) {
                    return std::string(reinterpret_cast<const char *>(
                        sqlite3_column_text(stmt, 0)));
                };
            std::vector<std::string> result_each_name =
                reopen_db.execute("SELECT * FROM names;", callback_each_name);
            ...
            SECTION("Rebuild existing database") {
                // setting build to true for an existing database will rebuild
                // the database
                database::Database db = database::Database(path, true);

                // callback function to capture the counts of executing the
                // query
                ...
                // query the sqlite_schema table to check that the database is
                // empty
                std::vector<int> result =
                    db.execute("SELECT COUNT(*) FROM sqlite_schema;", callback);
    ...
    SECTION("Build flags") {
        ...
        REQUIRE_THROWS_AS(database::Database(failpath).close(),
